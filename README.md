...
- ♻️ **跨平台图片去重**：除来源 ID 去重外，还会为 Pixiv / X / 抖音图片建立持久感知指纹；只有结构与低频颜色都严格一致的图片才自动去重，同轮待审冲突保留分辨率更高（同分辨率时文件更大）的版本；整图近似或“完整原图被拆成二/三分栏、2×2、3×3 局部图”的候选都不会自动删除，而是在审批卡片标出类型、双方来源、分辨率和文件大小
