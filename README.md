...
相似图审批整理历史频道作品时，如果 Telegram Bot API 无法删除超过 48 小时的旧帖，警告会列出精确频道链接并进入持久的手动清理状态。使用频道管理员账号删除全部链接后，依次点击 `✅ 我已手动删除频道消息` 与二次确认按钮；Hanabi 会重放并校验原始保留方案，跳过 Bot 删除步骤，完成 Vitrine receipt、本地整作品指纹清理和审批消息清理。手动清理状态在重启后仍可继续，原审批按钮不会误改已开始执行的取舍方案。
