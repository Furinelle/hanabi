...
- 🖼️ **人工审批**：作品先发审批私聊（单图直发 / 多图整组 + 控制消息），附 `✅ 发送到频道` / `📦 发送并入库`（配置 Shirogane 时）/ `❌ 丢弃`；同一按钮连点只会执行一次，第二次会显示「已在发布中」；点击后整组消息自动删除
- 📦 **图库入库**：可选对接 Cloudflare 图库 [shirogane](../shirogane)：审批「发送并入库」或手动链接直发频道成功后，用帖子自带标签写入图库
- ✅ **一键批准剩余项**：先逐条点 `❌ 丢弃` 排除不想推送的作品，再点命令菜单中的 `/approve`，按审批顺序发布所有剩余待审项，不必逐条批准；重复执行不会重复发布
...
> - **抖音图文**（短链 `v.douyin.com`，或「复制打开抖音…」整段分享文本）→ 解析下载**无水印原图**直发频道，`From 抖音 By 作者`。
...
> 推荐批量审批流程：先给不想推送的作品逐条点 `❌ 丢弃`，再从 Telegram 命令菜单点 `/approve`。命令会一次性原子抢占当时所有剩余待审项并按顺序发布；正在丢弃、单独批准或已被另一条 `/approve` 抢占的记录不会重复发布。单条发送失败会自动恢复为待审，可再次执行 `/approve` 重试。
