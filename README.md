...
相似图审批按候选图片逐张显示 `✅ 保留` / `❌ 丢弃`，相似旧图默认保留，并另设 `🗑 删除这张旧图`。按钮点击即生效，没有二次确认。最后一张候选审完后，自动把保留的图片发送到频道并入库；同一作品其余图片不受单张丢弃影响。`/undo` 恢复上一次点击前的选择；如果最后一张触发了发布，会先撤回该次发布与入库，再恢复审批状态。旧图删除只处理精确对应的一张频道图片和图库记录，撤销后恢复原图；Telegram 不支持原位恢复已删除的消息，因此恢复的频道图片会成为一条新消息。

旧图删除必须有完整、明确的频道消息映射。映射缺失时不执行删除；频道拒绝删除时自动恢复图库，失败恢复记录仍可通过 `/undo` 重试。历史整作品清理的手动完成按钮继续兼容，但也不再二次确认。已发布候选遗留的旧审批会转换成独立图片整理卡片。
