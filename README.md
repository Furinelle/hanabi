...
- 🪄 **原画质进评论区**：审批预览和频道帖复用 9.5 MB 内最高可上传质量的展示副本（透明 PNG 保留 alpha），**R2/本地原画质图不改写并作为文件自动投递到该帖评论区**（需频道绑定讨论组、bot 为管理员）；未绑定时自动降级，120s 兜底清理
- 🎨 **纯色图片过滤**：送审、直发和图库入库前跳过纯黑、纯白及其他纯色图片（容忍 RGB 通道 5 级以内的压缩噪声），保留同帖其余图片；纯色指纹不参与去重或相似审批。
...
- ↩️ **上一步可撤销**：`/undo` 撤回最近一次审批动作（发送到频道 / 发送并入库 / 丢弃），删掉刚发的频道帖和图库记录，并重新生成审批卡片；无本地文件的旧发布记录仍不可撤
...
图片指纹保存在同一个 `hanabi.db` 的 `image_fingerprints` 表。升级启动时会为仍存在原文件的旧待审项自动补建指纹；后续进入审批或手动直发成功的作品都会持续登记。手动链接遇到整部严格同图时，按总像素数、文件大小保留较高画质版本；若新版本更高，会先发布和入库新作品，再通过 Vitrine 回收凭据删除低清图库作品及对应频道帖。频道旧帖无法由 Bot 删除时会给出精确链接并进入持久手动清理，不会静默丢失其中一侧。

只命中某一页的严格同图、整图相似或局部相似都不会删单页。Hanabi 会把新链接的完整差分组和图库中每个匹配作品的完整差分组一起发到私聊；按钮按 `source:id` 整组取舍，选中的作品保留全部差分，落选作品同步从图库和频道清理。
...
| `/undo` | 撤销最近一次审批动作，重新生成该作品的审批卡片（保留 7 天） |
| `/status` | 待审数 + 运行状态 |
| `/ping` | 存活测试 |
| `/help` | 命令列表 |
...
| `/undo` 撤销上一步 | 撤回频道帖并恢复审批卡片 | 若刚入库则同时下线图库，恢复后由新选择决定 |
...
抓取 → 过滤/去重 → 下载 → 发审批私聊（图 + caption + 按钮）
        ...
        发布到频道            发布到频道+图库          （不发布）
              └─────────────────────┬─────────────────────┘
                           ...
                           删除私聊审批消息
                                    │
                         发送/入库/丢弃后 7 天内可用 /undo
```

相似图卡片标明原图分辨率与文件大小（不使用 Telegram 展示副本的压缩尺寸）。图库删除成功后移除对应私聊卡片，整组审批完成后清理其余卡片及控制消息；`/undo` 会重建所需卡片并保留原有撤销语义。

相似图审批按候选图片逐张显示 `✅ 保留` / `❌ 丢弃`，相似旧图默认保留，并另设 `🗑 删除这张旧图`。按钮点击即生效，没有二次确认。最后一张候选审完后，自动把保留的图片发送到频道并入库；同一作品其余图片不受单张丢弃影响。`/undo` 恢复上一次点击前的选择；如果最后一张触发了发布，会先撤回该次发布与入库，再恢复审批状态。旧图删除以精确对应的一张图库记录为准，同时尝试删除有明确映射的频道图片；频道删不了就跳过，不影响图库删除，也无需手动清理。

缺少频道消息映射时，只删除对应图库图片。`/undo` 会恢复图库；没有映射时不操作频道，原频道消息仍在时直接复用，确已删除的频道图片则重新发送为一条新消息。图库自身操作失败时保留恢复流程，仍可通过 `/undo` 重试。历史整作品清理的手动完成按钮继续兼容，但也不再二次确认。已发布候选遗留的旧审批会转换成独立图片整理卡片。

> 误点发送、入库或丢弃后发送 `/undo`，bot 会撤回刚产生的频道帖（入库的还会从 Vitrine 下线），保留原文件并重新生成一张可审批卡片。只撤销最近一步；更早的动作不会被误恢复。新的审批动作会替换旧的撤销记录并清理旧文件，最多保留 7 天。无本地文件的旧发布记录无法撤回。
